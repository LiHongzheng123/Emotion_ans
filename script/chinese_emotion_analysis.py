...
# @Software: PyCharm
# @function:利用最好的结果的分类器来进行情感的分类，即为多重贝叶斯。
...
pos_f = '../pkl_data/pos_review.pkl'
neg_f = '../pkl_data/neg_review.pkl'
...
def store_classifier():
    ...
    pickle.dump(MultinomialNB_classifier, open('../result/classifier.pkl', 'wb'))
...
def transfer_text_to_moto():
    ...
    moto = pickle.load(open('../pkl_data/test_review.pkl', 'rb'))  # 载入文本数据
...
def application(moto_features):
    clf = pickle.load(open('../result/classifier.pkl', 'rb'))  # 载入分类器
    pred = clf.prob_classify_many(moto_features)  # 该方法是计算分类概率值的
    p_file = open('../result/test_result.txt', 'w')  # 把结果写入文档
    for i in pred:
        p_file.write("正向概率为："+str(i.prob('pos')) + '    负向概率为：' + str(i.prob('neg')) + '\n')
    p_file.close()
